...
Class Consultas
{
	...
	{
		$sql="SELECT IFNULL(SUM(total_compra),0) as total_compra FROM ingreso WHERE estado = 'Aceptado' AND DATE(fecha_hora)=SUBDATE(CURDATE(), 1)";
		return ejecutarConsulta($sql);
	}

	public function totalventahoy()
	{
		$sql="SELECT IFNULL(SUM(total_venta),0) as total_venta FROM venta WHERE estado = 'Aceptado' AND DATE(fecha_hora)=SUBDATE(CURDATE(), 1)";
		return ejecutarConsulta($sql);
	}
